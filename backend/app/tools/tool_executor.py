...
def execute_tool_call(
    ...
    if tool_name not in allowed:
        logger.warning(
            "tool_call_rejected agent=%s tool=%s reason=not_allowed",
            agent_name,
            tool_name,
        )
        return {
    ...
    try:
        logger.info(
            "tool_execution_started agent=%s tool=%s",
            agent_name,
            tool_name,
        )
        ...
        logger.info(
            "tool_execution_finished agent=%s tool=%s status=success",
            agent_name,
            tool_name,
        )
        ...
        logger.exception(
            "tool_execution_failed agent=%s tool=%s",
            agent_name,
            tool_name,
        )
        return {
